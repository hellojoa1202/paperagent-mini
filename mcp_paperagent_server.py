...
from paperagent.workflow import PROTOTYPE_MAX_RUN_ATTEMPTS, run_pipeline  # noqa: E402
from paperagent.config import get_settings  # noqa: E402

mcp = FastMCP("paperagent-mini")
DEFAULT_OUTPUT_DIR = PROJECT_ROOT / "outputs"
# Change the version when the bundled UI changes.
APP_RESOURCE_URI = "ui://paperagent-mini/review-v16.html"
...
def get_paperagent_job_status(job_id: str) -> dict[str, object]:
    ...
    if not job:
        ...
        return {
            ...
            "prototype_attempt": int(checkpoint.get("prototype_attempt", 0)),
            "prototype_max_attempts": PROTOTYPE_MAX_RUN_ATTEMPTS,
        }
...
def _execute_paperagent_app(
    ...
    response = {
        ...
        "prototype_needs_review": result.prototype_needs_review,
        "prototype_attempt": int(
            _read_app_checkpoint().get("prototype_attempt", 0)
        ),
        "prototype_max_attempts": PROTOTYPE_MAX_RUN_ATTEMPTS,
        "papers": [
            {
...
def _render_mcp_response(result) -> str:
    prototype_line = (
        ...
        + (" — **점검 필요**" if result.prototype_needs_review else " — 검사 통과")
    ...
    return (
        ...
        "짧은 명사형 불릿으로 구조화하세요. `~합니다`, `~했다`, `~이다`, `~있다`처럼 "
        "문장으로 끝내지 마세요. Markdown 표 셀 안에서는 `<br>• `로 불릿을 구분하세요. "
        "모델명·방법명·분야 용어는 번역하거나 한글로 음역하지 말고 SOURCE의 영문 표기를 "
        "그대로 유지하세요. 예: anomaly, Transformer, attention, token merging, embedding, latency. "
        "단, 영어는 이러한 핵심 기술 용어에만 사용하고 제목을 제외한 모든 설명은 한국어로 "
        "작성하세요. 영문 문장 전체를 표에 넣지 마세요. "
