...
from paperagent.report_formatting import clean_agent_markdown, remove_visual_noise
from paperagent.report_quality import check_report
from paperagent.research_gap import append_next_experiments, run_research_gap_stage
...
class PipelineResult:
    ...
    prototype_needs_review: bool = False


PROTOTYPE_MAX_RUN_ATTEMPTS = 3
...
def run_pipeline(
    ...
    signature = {
        ...
        "enable_research_gap": enable_research_gap,
    }
            ...
            if enable_review:
                ...
                while (
                    ...
                    if verdict.score > best_verdict.score:
                        best_summary, best_verdict = summary, verdict
                summary, verdict = best_summary, best_verdict
    ...
    research_gap_text = str(state.get("research_gap_text", ""))
    research_gap_issues = tuple(state.get("research_gap_issues", []))
    if "research_gap" not in completed:
        if enable_research_gap and enable_literature_review and len(summaries) >= 2:
            _checkpoint_step(checkpoint, state, "research_gap")
            print("[7] ResearchGapAgent proposing next experiments")
            gap_source = _build_research_gap_source(literature_review, summaries)
            gap_result = run_research_gap_stage(
                topic=topic,
                literature_text=gap_source,
                count=settings.research_gap_experiment_count,
            )
            research_gap_text = gap_result.text
            research_gap_issues = gap_result.issues
            if research_gap_issues:
                print(
                    "[7] ResearchGapAgent output kept with validation warnings: "
                    + "; ".join(research_gap_issues)
                )
        elif enable_research_gap and len(summaries) < 2:
            print("[7] ResearchGapAgent skipped: at least two papers are required")
        state["research_gap_text"] = research_gap_text
        state["research_gap_issues"] = list(research_gap_issues)
        _complete_checkpoint_step(checkpoint, state, "research_gap")
        completed.add("research_gap")

    extra_reviews = [tuple(item) for item in state.get("extra_reviews", [])]
    if "extra_reviews" not in completed:
        if enable_extra_reviewers:
            _checkpoint_step(checkpoint, state, "postdoc_critic")
            print("[8] Running Experiment/Novelty/Impact reviewers")
            extra_reviews = [
    ...
    prototype_guide = ""
    prototype_path: Path | None = None
    prototype_needs_review = bool(state.get("prototype_needs_review", False))
    if enable_prototype and "prototype" not in completed:
        prototype_attempt = min(
            int(state.get("prototype_attempt", 0)) + 1,
            PROTOTYPE_MAX_RUN_ATTEMPTS,
        )
        state["prototype_attempt"] = prototype_attempt
        _checkpoint_step(checkpoint, state, "prototype")
        print("[9] Extracting methods and planning implementation")
        ...
        print("[10] PrototypeWriterAgent writing prototype.py")
        writer = PrototypeWriterAgent()
        generated_code = writer.generate_prototype_code(topic, plan_text)
        prototype_requirements = build_prototype_requirements(topic, plan_text)
        repaired_code, prototype_reviews = review_and_repair(
            generated_code,
            execute=settings.prototype_execute,
            max_repairs=settings.prototype_max_repairs,
            timeout=settings.prototype_timeout_seconds,
            requirements=prototype_requirements,
            topic=topic,
            implementation_plan=plan_text,
        )
        final_review = prototype_reviews[-1]
        if not final_review.passed:
            state["prototype_validation"] = {
                "passed": False,
                "repairs": len(prototype_reviews) - 1,
                "error": final_review.error,
                "needs_review": prototype_attempt >= PROTOTYPE_MAX_RUN_ATTEMPTS,
            }
            _save_checkpoint(checkpoint, state)
            if prototype_attempt < PROTOTYPE_MAX_RUN_ATTEMPTS:
                raise RuntimeError(
                    "프로토타입 코드가 품질·실행 검사를 통과하지 못했습니다. "
                    "Prototype 단계부터 다시 실행할 수 있습니다. "
                    f"({prototype_attempt}/{PROTOTYPE_MAX_RUN_ATTEMPTS})\n"
                    f"{final_review.error}"
                )

            storage_review = validate_code_for_storage(repaired_code)
            if not storage_review.passed:
                raise RuntimeError(
                    "3회 실행했지만 생성 코드에 문법 또는 위험한 호출 문제가 있어 "
                    "파일로 저장하지 않았습니다.\n"
                    f"{storage_review.error}"
                )
            prototype_needs_review = True
            print("[10] Prototype saved as an unverified draft after 3 attempts")
        else:
            prototype_needs_review = False
            print(
                f"[10] Prototype validation passed after "
                f"{len(prototype_reviews) - 1} repair(s)"
            )
        prototype_path = out_dir / "prototype.py"
        prototype_path.write_text(repaired_code, encoding="utf-8")
        ...
        state.update(
            ...
            prototype_needs_review=prototype_needs_review,
            prototype_validation={
                "passed": final_review.passed,
                "repairs": len(prototype_reviews) - 1,
                "error": final_review.error,
                "needs_review": prototype_needs_review,
            },
        )
    ...
    if enable_report and "professor" not in completed:
        ...
        print("[11] ProfessorAgent writing final synthesis")
    ...
    if research_gap_text:
        report_text = append_next_experiments(
            report_text,
            clean_agent_markdown(research_gap_text),
        )
    report_text = remove_visual_noise(report_text) + "\n"

    required_sections = ["Paper Summaries", "Literature Review"]
    if enable_review:
        required_sections.append("Summary Quality Review")
    if enable_critic and enable_literature_review:
        required_sections.append("Critical Review")
    if enable_report:
        required_sections.append("Final Synthesis")
    report_check = check_report(
        report_text,
        required_sections=tuple(required_sections),
    )
    state["report_quality"] = {
        "passed": report_check.passed,
        "missing_sections": list(report_check.missing_sections),
        "cjk_count": report_check.cjk_count,
        "duplicate_lines": list(report_check.duplicate_lines),
        "empty_sections": list(report_check.empty_sections),
        "broken_links": list(report_check.broken_links),
        "too_long": report_check.too_long,
    }
    if not report_check.passed:
        print("[12] Final report completed with deterministic quality warnings")
...
def _build_research_gap_source(
    literature_review: str,
    summaries: list[PaperSummary],
) -> str:
    """Give ResearchGapAgent exact references it can cite and validate."""
    paper_blocks = []
        ...
        paper_blocks.append(
            "\n".join(
                (
                    ...
                    f"- arXiv ID: {summary.paper_id}",
                    f"- Reviewed summary: {summary.summary}",
                )
            )
        )
    return "\n\n".join(
...
def _build_research_report(
    *,
    ...
    prototype_needs_review: bool,
) -> str:
    clean = clean_agent_markdown
    sections = [
        "# PaperAgent Research Report",
        f"- **Research topic**: {topic}",
        f"- **Paper count**: {len(summaries)}",
        "## 1. Paper Summaries",
        clean(write_paper_summaries(summaries)),
    ]

    if reviewer_feedback:
        sections.extend(["## 2. Summary Quality Review", clean(reviewer_feedback)])

    sections.extend(["## 3. Literature Review", clean(literature_review)])

    if critic_review:
        sections.extend(["## 4. Critical Review", clean(critic_review)])

    if extra_reviews:
        sections.append("## 5. Specialized Reviews")
        for title, text in extra_reviews:
            sections.extend([f"### {title}", clean(text)])
    ...
    if method_text or plan_text:
        sections.extend(
            [
                ...
                "## 6. 새로운 프로토타입 구현",
                (
                    "이 아래는 논문 분석과 구분하여 실제 프로토타입의 "
                    "구조와 구현 범위를 설명합니다."
                ),
                _build_prototype_framework(topic, prototype_path),
            ]
        )
        if method_text:
            sections.extend(["### Extracted Methods", clean(method_text)])
        if plan_text:
            sections.extend(["### Implementation Plan", clean(plan_text)])
        if prototype_path:
            validation_label = (
                "점검 필요" if prototype_needs_review else "검사 통과"
            )
            sections.extend(
                [
                    "### Prototype",
                    f"- Generated code: `{prototype_path}`",
                    f"- Validation: **{validation_label}**",
                    f"- Run: `python {prototype_path}`",
                ]
            )

    if professor_text:
        sections.extend(["---", "## 7. Final Synthesis", clean(professor_text)])

    report = "\n\n".join(
        section.strip() for section in sections if section.strip()
    )
    return remove_visual_noise(report) + "\n"


def _build_prototype_framework(
    topic: str,
    prototype_path: Path | None,
) -> str:
    generated_file = str(prototype_path) if prototype_path else "outputs/prototype.py"
    return f"""### 프로토타입 가안 제목

**{topic} 핵심 방법 검증 프로토타입**
...
| Core method | 논문의 핵심 계산을 작은 단위로 구현 | `{topic}` 구현 함수 |
...
| Output | 통과한 코드와 실행 결과 저장 | `{generated_file}` |
