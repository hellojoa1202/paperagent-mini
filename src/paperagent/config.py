"""Environment settings for PaperAgent."""
...
class Settings:
    ...
    research_gap_experiment_count: int = 3
    prototype_max_repairs: int = 2
    prototype_execute: bool = True
    prototype_timeout_seconds: int = 10
...
def get_settings() -> Settings:
    ...
    return Settings(
        ...
        research_gap_experiment_count=int(os.getenv("RESEARCH_GAP_EXPERIMENTS", "3")),
        prototype_max_repairs=int(os.getenv("PROTOTYPE_MAX_REPAIRS", "2")),
        prototype_execute=os.getenv("PROTOTYPE_EXECUTE", "true").strip().lower()
        in {"true", "1", "yes"},
        prototype_timeout_seconds=int(os.getenv("PROTOTYPE_TIMEOUT_SECONDS", "10")),
    )
