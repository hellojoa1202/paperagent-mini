...
def run_pipeline(
    ...
    if enable_prototype and "prototype" not in completed:
        ...
        prototype_code = writer.generate_prototype_code(topic, plan_text)

        execute_val = settings.prototype_execute
        max_repairs = settings.prototype_max_repairs
        print(f"[9.1] Validating prototype code (execute={execute_val}, max_repairs={max_repairs})")

        reviewer = PrototypeReviewerAgent()
        current_code = prototype_code
        for attempt in range(max_repairs + 1):
            review = validate_code(current_code, execute=execute_val)
            if review.passed:
                print(f"[9.2] Prototype code validation passed (attempt {attempt + 1})")
                break
            if attempt == max_repairs:
                print(f"[9.2] Prototype code validation failed after {attempt} repairs: {review.error}")
                break
            print(f"[9.2] Validation failed: {review.error}. Repairing (attempt {attempt + 1}/{max_repairs})...")
            current_code = reviewer.repair(current_code, review)

        prototype_path = out_dir / "prototype.py"
        prototype_path.write_text(current_code, encoding="utf-8")
