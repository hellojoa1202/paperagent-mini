...
위 계획을 구현한 바로 실행 가능한 Python 코드를 작성해 주세요.
작성 조건:
1. 외부 API 호출이나 로컬 파일 로드 없이 작동해야 합니다. 스크립트 내부에 mock data를 직접 생성하여 사용해 주세요.
2. 표준 라이브러리(math, json, random, re, collections 등)와 NumPy만 사용해서 가볍게 만드세요.
3. 함수와 클래스의 기능은 구현 생략(pass 등) 없이 작동할 수 있는 최소한의 코드로 구현하세요.
4. 예외나 불필요한 복잡성은 줄이고 직관적인 흐름으로 작성하세요.
5. 설명 없이 markdown 코드 블록(```python) 하나에 Python 코드만 담아주세요.
6. 마지막에 `if __name__ == "__main__":` 실행 블록을 포함하여 실행 시 전체 흐름을 테스트하고 결과를 출력하도록 하세요.
"""
...
class PrototypeReviewerAgent(BaseAgent):
    """Senior Python engineer reviewing and repairing prototype code errors."""

    name = "PrototypeReviewerAgent"
    role = "senior Python engineer repairing a small generated prototype"

    def repair(self, code: str, error: Any) -> str:
...
발생한 오류를 해결하도록 최소한의 코드 수정을 진행해 주세요.
표준 라이브러리 또는 NumPy 외의 외부 라이브러리는 사용하지 않아야 하며, 외부 데이터 로드나 API 호출 없이 즉시 실행 가능해야 합니다.
설명이나 주석 없이 markdown 코드 블록(```python) 하나에 수정된 Python 전체 코드만 담아주세요.
"""
